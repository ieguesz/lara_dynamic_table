...
@section('content')
<?php
$Ccabecera = json_decode($cabecera, true);
$permisoId = $Ccabecera['id'];
$estado = $Ccabecera['per_estado'];
				...
				<div class="card card-secondary">
					<div class="card-header">
						<div class="row">
							...
							<div class="col-3">
							<a id="P_agregar" class="btn btn-primary
							{{ $disabled }}"><i class="fa fa-save"></i> Agregar Permiso</a>
							</div>
						</div>
					</div>
					<!-- /.card-header -->
					...
					<div class="card-body">
						...
						<div class="modal-footer">
							<a id="P_edit" href="{{ route('access.permiso.edit',$permisoId) }}" class="btn btn-primary" style="display: none"><i class="far fa-credit-card fa-2x"></i> Nuevo Permisos
                            </a>
							<a id="P_cancelar"type="button" class="btn btn-danger"
							href="{{route('access.permiso.index')}}
							"><i class="fa fa-times fa-2x"></i> Cancelar </a>

							<button id="P_guardar" class="btn btn-primary" id="genera_movimiento" {{ $disabled }}><i class="fa fa-save fa-2x"></i> Guardar </button>
						</div>
					</div>
				</div>
			</div>
...
// "use strict"
const cabecera = <?php echo $cabecera; ?>;
const detalles = <?php echo $detalle; ?>;
...
$(document).ready(function() {
	...
	$("#frm-create").on('submit',function(e) {
        ...
        const url = "{{route('access.permiso.update', $permisoId)}}";
        var formData = new FormData();
        formData.append('id_usuario', $("#P_descripccion_usuario").val());
        formData.append('id_delete',JSON.stringify(deleteIdTable));
...
//publicar_datos_editar
	var newOption = new Option(cabecera.user_descripcion, cabecera.id_user, true, true)
	$('#P_descripccion_usuario').append(newOption).trigger('change')
	$('#P_nombreUser').val(cabecera.user_descripcion)
	$('#P_codigoUser').val(cabecera.id_user)
	// debugger
	for (var i = 0;  i < detalles.length ; i++) {
		addRowTable(
				detalles[i].id,
				detalles[i].id_menu,
				detalles[i].menu_descripcion,
				0
			);
	}
...
var count = 1
const addRowTable = (detalleId, menuId, menuNombre, nuevo) => {
	if(!esMenuRepetido(menuId)){
		let row = `<tr>
                   <td data-id_detalle="${detalleId}" data-id_menu="${menuId}" data-new=${nuevo} ><h6>${count}</h6></td>
                   <td><h6>${menuNombre}</h6></td>
                   <td>
                       <center>
                       <h6>
                           <a class="btn btn-primary btn-sm p_remover {{ $disabled }}"><i class='fa fa-times'></i></a>
                       </h6>
                       </center>
                   </td>
                   </tr>`;
...
$(document).on('click', '.p_remover', function(e){
    ...
    if($(this).parents("tr").find('td').eq(0).attr('data-new') != 1){
    	let idDelete = $(this).parents("tr").find('td').eq(0).attr('data-id_detalle')
    	deleteIdTable.push(idDelete)
    	console.log(deleteIdTable)
    }
    // remueve la fila
